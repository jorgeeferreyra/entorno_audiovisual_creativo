...
**Método:** instancia del paso 3 del pipeline ([../../../metodo/pipeline.md](../../../metodo/pipeline.md) §2). **Goal:** las madres del reel comparten un solo universo visual por registro (cuento/Reiniger y real/Revenant) — misma paleta exacta, textura de papel y peso de línea/filigrana (o grano/color science documental), sin aplanar el tinte del beat ni los encuadres FLF. **Scope:** [reels/la-grieta/mapa-uniformidad.md](reels/la-grieta/mapa-uniformidad.md) (locks + mapa) → `npm run uniformar -- --reel la-grieta` → carpeta [`_madres-uniformes/`](reels/la-grieta/_madres-uniformes/). Nunca pisa canónicos de `assets/arco-N/madre/`. **Dependencies:** Stage 3 (Imágenes madre) — las madres base ya aprobadas. **Gate (secuencia de confirmaciones):** (1) locks designados → (2) mapa aprobado → (3) comando corrido → (4) carpeta `_madres-uniformes/` aprobada. La promoción a canónicos (`--promover`, archiva originales en `_prev/`) es un paso posterior con confirmación explícita. Orden recomendado con el gate hermano de aspecto 9:16: uniformar → aspecto. **Exit:** `_madres-uniformes/` aprobada; criterio de la etapa Madres uniformes en [PROGRESS.md](PROGRESS.md).
