...
# A2 (beat 9) + coda A2 (beat 13), ya bajados a planos/arco-1.md y planos/arco-2.md.
cutlist:
  ...
  - { clip: a1-a1,  dur: 2.5 } # A1 REVEAL: la mano firma (gap)
  ...
  - { clip: a3-a6e, dur: 1.5 } # beat 8.2c lejano los 4 + grieta (still m24)
  ...
  - { clip: a2-a0c, dur: 1.5 } # beat 9.2 golpe de la grieta, Revenant (still a2-m07)
  - { clip: a2-a0d, dur: 1.5 } # beat 9.2 vuelta al cuento, Reiniger (still a2-m08) — cierra el switch
  - { clip: a2-a1,  dur: 1.5 } # beat 9.4 la pisada (still a2-m02a)
  - { clip: a2-a1c, dur: 1.5 } # beat 9.4 el nido vacío (still a2-m02b)
  - { clip: a2-a1b, dur: 2 }   # beat 9.4 el huevo cruza (FLF a2-m02d→a2-m02c)
  - { clip: a2-a0b, dur: 1.5 } # beat 9.3 levanta la cría (still a2-m03) — cierra el zoom 9
  - { clip: a3-b4,  dur: 2 }   # Argentina seca (FLF) — cierra beat 9.6 (consecuencia)
  - { clip: a3-c1,  dur: 2 }   # el último / rocas (regen)
  - { clip: a3-c2,  dur: 2 }   # fosilización (FLF)
  - { clip: a2-a2,  dur: 1.5 } # beat 13.2 lugar blanco: "es en Rocas Coloradas" (still a2-m04)
  - { clip: a2-a2b, dur: 1.5 } # beat 13.3 despertar, ojos Coloradas (still a2-m05)
  - { clip: a2-a2c, dur: 1.5 } # beat 13.3 despertar, selva (still a2-m09) — cierra la coda
  - { clip: a3-c3,  dur: 2 }   # el fósil hoy (gen)
  - { clip: a3-c4,  dur: 2 }   # foto real + Fundación
---
...
- **Arcos que cruza**: `arcos: [1, 2, 3]`. La `cutlist` del front-matter es el mapa de intercut (~55s) de la [auditoría §A](../../auditoria-modelo-transversal.md): espina emocional del Arco 3, reveal del Arco 1 (`a1-a1`, la mano firma) tras la grieta, contrapunto del Arco 2 (beat 9, la palanca) en vidas paralelas. Los zooms de los beats 8 (separación: `a3-a6a`…`a3-a6e` — progresión grieta + orillas + lejano), 9 (Charles/palanca: `a2-a0` → `a2-a0c`/`a2-a0d` (switch grieta real→cuento) → `a2-a1`/`a2-a1c`/`a2-a1b` (FLF) → `a2-a0b`) y 13 (coda del lugar blanco → despertar: `a2-a2`/`a2-a2b`/`a2-a2c`, entre `a3-c2` y `a3-c3`) entran como stills de montaje (~1.5–2s); el animatic decide si alguno asciende a clip U2V o se recorta.
