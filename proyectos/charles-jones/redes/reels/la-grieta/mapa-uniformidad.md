...
madres:
  ...
  - { id: a2-m07, lock: real, esLock: true, fuente: "assets/arco-2/madre/a2-m07-grieta-revenant.png", tinte: "deep blood-red overcast dusk light", preserva: "universe lock real — SAME framing as a2-m08, extreme OTS", crop: 360 }
  ...
  - { id: a3-m14, exento: true, motivo: "reemplazada por a2-m07 como lock real / eco a3-c5; canónico eliminado a propósito" }
  ...
  # --- Resueltas 2026-07-14 (ya no diferidas; grade+crop capa 1) ---
  - { id: a3-m01v2, lock: cuento, tinte: "cold desaturated grey tinted background", preserva: "mother silhouette fidelity; cracked dry plain; exhausted posture" }
  - { id: a2-m02d, lock: cuento, tinte: "dramatic deep red tinted background with living green vegetation accents", preserva: "canónico = copia de a2-m02c (dirección 2026-07-14)" }
  - { id: a1-m01a, lock: cuento, tinte: "dramatic deep red tinted background", preserva: "SAME framing as a1-m01; signature-zone tear only (FLF pair)" }
---
...
7. **`aspecto: nativo`**: conserva dims originales (sin crop ni marca de outpaint). Pensado para locks que no deben recibir píxeles generados; los usos de cutlist pueden croppear aparte.
...
| Lock real | `a2-m07` (canónico `a2-m07-grieta-revenant.png`) — reemplaza a `a3-m14` (eliminada) |
...
| Outpaint Fase 2 | **RESUELTO 2026-07-14**: `a3-m03`←c1, `a3-m22`←c2, `a1-m01`←c2 (2:3→crop 9:16 en capa 1) |
...
| Exentas | `a3-m12` (marco POV). `a3-m14` (reemplazada por `a2-m07`). `a3-c4` no es madre. Eco `a3-c5` = reprise `a2-m07` |
...
Se generan post-promoción con `ref` a canónicos ya uniformes: `a1-m01a`, `a2-m02d`, `a3-m01v1`, `a3-m01v2`, `a3-m15v1`. `a3-m05v1` queda cancelada por la v3: al caer `a3-a2`, `a3-m05` ya no se reutiliza en pantalla y puede ser el firstFrame directo de `a3-a5`.
