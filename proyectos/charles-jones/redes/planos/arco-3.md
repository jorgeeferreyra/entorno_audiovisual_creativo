...
ref: assets/arco-3/madre/_pre-gate/a3-m24-tanda-reiniger-c3.png
anatomyRefs:
...
Nota: establishing lejano — base aprobada de dirección: `_pre-gate/a3-m24-tanda-reiniger-c3.png` (Reiniger, los cuatro ya presentes). **Delta único:** separar un poco los dos grupos — alejarlos del borde de la grieta / ampliar el aire entre orillas, sin cambiar estilo, vegetación ni cascada. AnatomyRefs `m22`/`m23` solo identidad de contorno a escala pequeña. Pick canónico: `a3-m24-c1` (tanda separar grupos; base `tanda-reiniger-c3`).

Prompt (EN):
```
The first reference image is the APPROVED base composition — keep it with near-100% fidelity: the same Lotte Reiniger paper cutout style, the same stacked black silhouette layers, the same lush silhouetted ferns and tropical foliage framing the scene, the same two cliffs with the glowing red-orange chasm and waterfall between them, the same deep red tinted sky with distant cutout mountains, and the same four tiny platypus-family silhouettes — left shore: larger rounded mother + smaller young; right shore: bulkier father + small egg nest. Do NOT restyle, do NOT reframe, do NOT zoom, do NOT change the waterfall, vegetation, or torn-paper mood. The ONLY change: separate the two groups a little — pull both family groups slightly back from the cliff rims so there is more breathing room across the gap; their snouts must NOT nearly meet; the irreversible distance between the shores should read clearly, while all four figures stay readable as tiny backlit silhouettes. The second reference (mother + young) and third reference (father + egg nest) are ONLY for silhouette identity at tiny scale, never copy their close framing. CRITICAL: exactly FOUR tiny black platypus-family silhouettes, same identities as the base, just a bit farther apart across the chasm, no other characters, no other changes, dramatic deep red tinted background, Lotte Reiniger inspired paper cutout silhouette animation, shadow puppet theater, black silhouettes with delicate cut-out inner details, tinted aged-paper background, dark fairy tale mood, sepia edges, vertical 9:16
```
