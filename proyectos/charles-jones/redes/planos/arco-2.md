...
Registro del Arco 2: esperanzador, el contrapeso del Arco 1. En el reel entra como **contrapunto** durante "vidas paralelas": la palanca de Charles explica por qué la línea australiana sobrevivió. Se produce mayormente en **silueta** Lotte Reiniger. **Guarda de cara:** en registro **Revenant / photoreal** Charles nunca de frente (de espaldas, OTS, manos); en registro **Reiniger** la cara **puede verse** en perfil/tres cuartos recortado (ver [personajes-studio.md](../../personajes-studio.md)). **Excepciones Revenant (decisión de dirección):** (1) el golpe de la grieta — **switch transversal de ida y vuelta**: `a2-m07` (real, `styleBlock: false`, hermano de `a3-m14`) → `a2-m08` (mismo encuadre, vuelta al cuento Reiniger); patrón "primero sucede en Revenant y se convierte a Reiniger" que se repite transversalmente en el arco; (2) la coda del lugar blanco (`a2-m04`) en void blanco puro. El master de silueta (`a2-m01`) sostiene el hilo en cuento; la coda del despertar es **en secuencia**: ojos Coloradas `a2-m05` (Reiniger, ECU) → selva post-sueño `a2-m09` (Reiniger, cara visible), empalmando recién ahí con el Bloque C.

## Guion de color (tinte de fondo por beat)

| Beat | Tinte (línea EN del prompt) |
|---|---|
| La palanca (a2-m01, a2-m02a, a2-m02b, a2-m02c, a2-m02d, a2-m03, a2-a1, a2-a1c, a2-a1b, a2-a0b) — en el momento de la grieta | `dramatic deep red tinted background` |
| La grieta Revenant (a2-m07, a2-a0c) — golpe del quiebre; primer eslabón del switch transversal (real → cuento) | REALITY-BLOCK Revenant, `deep blood-red overcast dusk light` (`styleBlock: false`; desvío rojo del bloque canónico) |
| La grieta Reiniger (a2-m08, a2-a0d) — mismo encuadre; segundo eslabón, vuelta al cuento | `dramatic deep red tinted background` |
...
**a2-m02a — La pisada (beat 9.4) — un sujeto**

```yaml
kind: image
dest: assets/arco-2/madre/a2-m02a-pisada.png
ref: a2-m03
anatomyRefs:
  - assets/fuentes/charles/charles-jones-referencia.jpeg
provider: openrouter
...
**a2-m02b — El nido vacío (beat 9.4) — un sujeto**

```yaml
kind: image
dest: assets/arco-2/madre/a2-m02b-nido-vacio.png
ref: a2-m02a
anatomyRefs:
...
Nota: **un sujeto** — solo el nido vacío sobre el tronco (eco: él se llevó a la cría; el huevo queda aparte). Still de montaje (`a2-a1c`, ~1.5s) entre la pisada y el FLF del huevo. `ref: a2-m02a` hereda mundo/tronco; `a3-m04` aporta la forma del nido (sin el huevo).
...
**a2-m02c — El huevo del otro lado (keyframe final FLF, beat 9.4) — diferido a keyframes**

```yaml
kind: image
dest: assets/arco-2/madre/a2-m02c-huevo-otro-lado.png
anatomyRefs:
  - assets/arco-3/madre/a3-m04-huevo.png
...
Nota: **keyframe final** del par FLF con `a2-m02d` (`a2-m02d` → `a2-m02c`, clip `a2-a1b`) — **ya no es el mismo encuadre** (aterrizaje, no balancín). El huevo quedó **del lado Australia**, claramente de un solo lado: **acostado de costado**, en ángulo desparejo, como tumbado por el envión (no acomodado). **Sin tronco, sin grieta, sin portal rojo en V.** Suelo/orilla próspera con helechos verdes (eco de `a3-m08`); tinte rojo del beat se mantiene en el fondo. `a3-m04` = master del huevo; `a3-m08` = solo mood de vegetación viva, no copiar el canal de agua ni el pájaro. Sin bota, sin nido. **Solo ficha** — se genera en el gate de madres keyframes.
...
**a2-m02d — El huevo de este lado (keyframe inicial FLF, beat 9.4) — diferido a keyframes**

```yaml
kind: image
dest: assets/arco-2/madre/a2-m02d-huevo-este-lado.png
ref: a2-m02a
anatomyRefs:
  - assets/arco-3/madre/a3-m04-huevo.png
provider: openrouter
...
Nota: Pick canónico: `a2-m05-c3` → `a2-m05-despertar.png`. **Primer paso** de la coda del despertar (secuencia `a2-m05` → `a2-m09`); el empalme con el Bloque C lo cierra recién `a2-m09`. **Registro Lotte Reiniger**, no Revenant. **Plano extremo de los ojos** (ECU): cara visible en filigrana de tijera (excepción Reiniger). **Ref 1** (pintura Charles): identidad de ojos, cejas, ala del sombrero, barba. **Ref 2** (`a3-m07`): mundo de recortes rojos de las Coloradas. Gesto: párpados que se abren / mirada que enfoca, como quien vuelve de un trance. Tinte rojo dusk (puente de color al Bloque C).
...
Nota: Pick canónico: `a2-m07-c2` → `a2-m07-grieta-revenant.png` (**canónico borrado, re-pick pendiente** — solo queda en disco la segunda aprobada `a2-m07-c3`, OTS más pegado, en `a2-m07-grieta-revenant-c3.png`). **Iteración de dirección:** ambas quedaron serenas (postal); el prompt se reescribió para describir el colapso **en acción** (bordes desmoronándose, agua que explota en spray, columnas de polvo, bandadas huyendo, dutch angle) — más realismo caótico. Pendiente re-pick sobre los nuevos candidatos antes de completar el slot canónico. **quiebre fotorrealista Revenant con Charles presente** — el golpe visceral del beat de la palanca, hermano de `a3-m14` (grieta photoreal) pero con la figura en cuadro. Encuadre **OTS extremo**: cámara casi pegada a la nuca — ala del sombrero y hombro ocupan un tercio o más del cuadro, desenfocados por cercanía (foreground bokeh). Charles parado **elevado en una colina/pendiente, lejos** de la destrucción; la laguna que drena y la grieta quedan abajo en el valle, chicas en el cuadro pero de escala colosal (panorama, bruma, escombros lejanos). La enormidad se lee por perspectiva, no por proximidad. El caos es el escudo de las guardas (cara oculta, manos fuera de cuadro). **Desvío controlado del REALITY-BLOCK-CHAOS:** luz de atardecer rojo profundo en vez de `cold muted earth tones`, para sostener el mundo rojo del Arco 2. Solo ref de foto (`charles-jones-referencia.jpeg`) para vestuario, sin la pintura. **Primer eslabón del switch transversal** con `a2-m08` (mismo encuadre en Reiniger): el golpe sucede primero en Revenant y se convierte a Reiniger — no es una elección A/B, son dos pasos secuenciales del mismo golpe (clips `a2-a0c` → `a2-a0d`). A futuro puede ascender a par FLF real→dibujo en el gate de madres keyframes.
...
**a2-m08 — La grieta (Reiniger, mismo encuadre) — segundo eslabón del switch**
...
Nota: Pick canónico: `a2-m08-c3` → `a2-m08-grieta-reiniger.png`. **Segundo eslabón del switch transversal** con `a2-m07` — mismo encuadre (OTS extremo + Charles elevado en colina + grieta/laguna como panorama lejano) traducido a silueta Lotte Reiniger: el golpe sucede primero en Revenant (`a2-m07`) y se convierte acá de vuelta al cuento. Es el patrón de switch de ida y vuelta de estilo que se repite transversalmente en el arco (ver [biblia-visual.md](../../biblia-visual.md) §1 "Quiebres de realidad / switches cuento↔real"), no una alternativa A/B. No toca `a2-m04` (Revenant lugar blanco) ni `a2-m05`/`a2-m09` (coda del despertar). Capas de teatro de sombras: hombro/ala del sombrero como capa negra grande en primerísimo plano (filigrana Reiniger, no vector plano), Charles en lo alto de una loma como capa media, valle verde con laguna drenando en la grieta como capas chicas al fondo. Guarda dura: **nunca de frente**.
...
- Audio: off en primera persona (beat 3.1 v4: "Estaba todo muy inestable.") — ver [arco-2-off.md](arco-2-off.md)
- Montaje (reel v4): tras el crimen (`a1-a1` → `a3-a5`); still de m01 (silueta de espaldas); corta a la pisada `a2-a1`. El animatic decide si asciende a clip U2V.
...
duration (montaje): 1–1.5s
- Audio: off en primera persona (beat 1.1 v4 · **HOOK**: "Yo estuve ahí…") — ver [arco-2-off.md](arco-2-off.md)
- Montaje: still de m07 (OTS extremo fotorrealista, colapso en acción); **abre el reel v4** como hook absoluto. Primer eslabón del switch transversal real→cuento. Corta a `a2-a0d` (vuelta al cuento). El animatic decide si asciende a clip U2V o si el par sube a FLF real→dibujo en el gate de keyframes.
...
duration (montaje): 1–1.5s
- Audio: *(silencio)* — transición muda al cuento
- Montaje: still de m08 (mismo encuadre en silueta); **segundo eslabón**, cierra el switch iniciado en `a2-a0c` y devuelve el registro al cuento. En el reel v4 corta a `a1-a1` (el crimen firmado, beat 2).
...
duration (montaje): 1–1.5s
- Audio: off en primera persona (beat 3.2 v4.1: "Por mi descuido…") — ver [arco-2-off.md](arco-2-off.md)
- Montaje: still de m02a (solo la bota sobre el tronco); corta a `a2-a1c` (nido vacío).

**Clip a2-a1c — El nido vacío (montaje, sin generación) — beat 4.2**
...
- Montaje: still de m02b (solo el nido vacío sobre el tronco); corta a `a2-a1b` (el huevo cruza).

**Clip a2-a1b — El huevo cruza (beat 4.3)**
...
cameraPreset locked-tripod: el mundo de recortes es plano. **Composición diverge** entre frames (m02d = tronco/este lado; m02c = aterrizaje Australia, sin tronco ni grieta) — el FLF morphs el cruce; aprobar el par junto en el gate de madres keyframes (pipeline §2 paso 4) antes de pagar este clip. FLF no acepta 6s.
...
- Audio: off en primera persona (beat 3.4 v4: "Una vida cruzó el planeta.") + música tenue — ver [arco-2-off.md](arco-2-off.md)
- Montaje: tras el nido vacío; corta a `a2-a0b` (alzada de la cría). Orden del beat 3 en el reel v4: `a2-a0` → `a2-a1` → `a2-a1c` → `a2-a1b` → `a2-a0b` (antes de la separación, beat 4).

**Clip a2-a0b — Las manos levantan a la cría (montaje, sin generación) — cierre del beat 3**
...
duration (montaje): 1–1.5s
- Audio: off en primera persona (beat 3.5 v4.1: "Otra vida quedó de este lado.") — espejo del huevo; sale la amistad
- Montaje: still de m03 (manos alzando la cría); cierra el beat 3. En el reel v4 corta directo a `a3-a6c` (familias separadas, beat 4).
...
```yaml
kind: montaje
fuente: assets/arco-2/madre/a2-m04-lugar-blanco-c3.png
duration: 1.5
```
