...
4. **Coherencia entre madres**: las madres de un mismo beat/locación comparten composición base (encuadre, capas, posición del sujeto) — la continuidad de los clips se hereda de esa coherencia. Las madres **emparejadas** (par first/last de un clip FLF) son la forma fuerte: mismo encuadre exacto, solo cambia el estado. Ejemplos canónicos: `a3-m05`→`a3-m06` (Pangea → Pangea partida), `a3-m10`→`a3-m11` (ornitorrinco vivo → fósil; la Ref de generación puede ir en la dirección que preserve la madre ya aprobada — hoy `m10` hereda de `m11`). La forma **global** (mismo universo visual entre arcos/registros del reel) es el gate de uniformidad de universo ([pipeline.md](../../metodo/pipeline.md) §2 paso 3; mapa en `reels/<slug>/mapa-uniformidad.md`).
5. **Secuencia de madres por clip (keyframes)**: la regla general (cuántas madres pide cada escena — 1 / 2 / N keyframes, la técnica de cadena de eslabones FLF con keyframe compartido, la regla anti-inflación de costo y el gate propio de pares/cadenas aprobados juntos) vive en [pipeline.md](../../metodo/pipeline.md) §2 paso 5 y no se duplica acá. Aplicación en el Arco 3: los pares emparejados de la regla 4 (`a3-m05`→`a3-m06`, `a3-m10`→`a3-m11`) son los casos de 2 keyframes; las cadenas concretas del arco viven en [arco-3-planos.md](redes/planos/arco-3.md) §Cadena de transiciones.
...
7. **Unicidad por escena (madres variations)**: una imagen madre repetida tal cual en varias escenas se nota — el espectador registra el frame reutilizado. La **madre base** es el canon estético (identidad aprobada); su **primera aparición** en pantalla puede usar la base, pero **toda reutilización dispara una variación**: una instancia única del mismo elemento, derivada de la base con el mecanismo de la regla 6 (`ref: a3-mNN` + `provider: openrouter`, Nano Banana), con fidelidad 100% al estilo y un delta solo de pose/encuadre/estado y tinte del beat destino. El ID lleva sufijo `v{k}` (`a3-m01` → `a3-m01v1`, `a3-m01v2`, ver [pipeline.md](../../metodo/pipeline.md) §5). **Exenciones** (la repetición es el mecanismo, no un descuido): el **keyframe compartido** de una cadena FLF (el `lastFrame` de un eslabón es el `firstFrame` del siguiente — variarlo rompe la unión invisible) y el **eco deliberado** (repetir una madre como flashback/puente emocional intencional, ej. el eco de `a3-m09` en `a3-c3e`). Gate: el animatic se corre con las variaciones ([pipeline.md](../../metodo/pipeline.md) §2 paso 4).
