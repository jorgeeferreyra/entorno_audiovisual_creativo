...
>
> - Flujo de producción, estrategia "un personaje por clip" y presupuesto del proyecto: [pipeline-wind-comic.md](pipeline.md).
> - Anclas de estilo y prompts madre del proyecto: [biblia-visual.md](../proyectos/charles-jones/biblia-visual.md).
> - Grado de aprovechamiento de este alcance ("brillo") y cómo se mide: [brillo-wind-comic.md](brillo.md).
...
**Modos de desarrollo local** (detalle completo en [pipeline-wind-comic.md](pipeline.md)): `MOCK_ENGINES=1` genera salidas fake sin gastar en APIs; `PLAN_GATE_DISABLED=1` desbloquea funciones sin pago a la app (los gates son de la versión SaaS, no aplican a la instancia local).
