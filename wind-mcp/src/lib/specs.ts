 ...
 *
 * Fuente de verdad: docs/produccion/arco-{N}-planos.md. Cada ficha declara sus
...
/**
 * Resuelve una referencia (ref/firstFrame/lastFrame/fuente) a ruta relativa a
 * PROJECT_ROOT. Si tiene forma de id de asset (`a{arco}-...`), exige que exista
...
export async function leerPlanos(arco: number): Promise<AssetSpec[]> {
  const planosMd = path.join(PROJECT_ROOT, 'docs', 'produccion', `arco-${arco}-planos.md`);
