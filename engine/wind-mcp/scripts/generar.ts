...
async function main() {
  ...
  // Listado (sin filtros).
  if (!id && !reel && !todas) {
    console.log(`Assets del Arco ${arco} (fuente: planos/arco-${arco}.md):\n`);
  ...
  if (!estado.serverUp) {
    throw new Error('wind-comic no está arriba. Levantá: cd engine/wind-comic && PLAN_GATE_DISABLED=1 npm run dev');
  }
  ...
  if (id) {
    const spec = specs.find((s) => s.id === id);
    if (!spec) throw new Error(`Asset ${id} no encontrado en planos/arco-${arco}.md`);
    seleccion = [spec];
  } else if (reel) {
