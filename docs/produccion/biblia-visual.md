...
6. **Mecanismo de la coherencia entre madres emparejadas**: la madre hija se genera usando la madre padre (ya aprobada) como **referencia de imagen** (`subject_reference`), no solo describiéndola en el prompt — describir "the same landscape..." sin una referencia real es frágil, el modelo reinventa el encuadre en cada corrida. La relación se declara en [arco-3-planos.md](arco-3-planos.md) con el campo `- Ref: a3-mNN` en la ficha de la madre hija; `wind-mcp/scripts/generar-madres-a3.ts` la resuelve automáticamente (data-driven, sin flags manuales).
